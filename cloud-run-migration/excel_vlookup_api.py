...
document.addEventListener('pointerup',e=>{if(!swipeActive||e.pointerType!=='touch')return;swipeActive=false;let dx=e.clientX-swipeStartX,dy=e.clientY-swipeStartY;if(dx<-45&&Math.abs(dx)>Math.abs(dy)*1.1)goPreviousPage()},{passive:true});
