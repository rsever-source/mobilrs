...
function buttons(){let bs=allBrands();if(activeBrand&&!bs.includes(activeBrand))activeBrand=null;document.getElementById('otvChips').innerHTML=bs.map(b=>'<button class="chip '+(b===activeBrand?'on':'')+'" onclick="setBrand('+JSON.stringify(b)+')">'+b+'</button>').join('')}
