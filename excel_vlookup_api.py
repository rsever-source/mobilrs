...
from vehicle_images import get_vehicle_image, vehicle_image_url
...
};let key=b+'|'+m;for(let k of Object.keys(pool)){if(key===k||((k.endsWith('|EGEA')||k.endsWith('|EGEA CROSS'))&&key===k)||(!m&&b===k.split('|')[0]))return pool[k]}for(let k of Object.keys(pool)){let parts=k.split('|');if(parts[0]===b&&m.includes(parts[1]))return pool[k]}return ''}function verifiedRow(v){let ep=exemptPrice(v),img=vehicleImage(v);return `<div class="row"><div class="rowtop"><div class="vehicle-main"><div class="vehicle-mini">${img?`<img src="${img}" loading="lazy" alt="${v.brand} ${v.model}" onerror="this.parentElement.textContent='🚗';">`:'🚗'}</div><div><div class="name">${v.brand} ${v.model}</div><div class="meta">${v.trim||''}</div></div></div><span class="tag">Uygun</span></div><div class="price">Liste Fiyatı<b>${tl(v.price)}</b></div>${ep?`<div class="price">Tahmini ÖTV Muaf Fiyat<b>${tl(ep)}</b></div>`:''}<div class="note">Yerlilik: %${v.locality||'—'} · Kaynak: ${v.source_name||v.brand} · Son kontrol: ${v.checked_at||'—'}</div></div>`}
