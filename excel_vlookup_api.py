...
def check_extension(filename, allowed):
    return (filename or "").lower().endswith(allowed)


def normalize_column_name(v):
    return str(v).strip()
...
def select_join_column(a, b, command):
    ...
    if not common:
        raise HTTPException(400, "İki Excel dosyasında ortak sütun bulunamadı.")
    cl = command.lower()
    for c in common:
        if str(c).lower() in cl:
            return c
...
def find_command_columns(df, command):
    cl = command.lower()
    return [c for c in df.columns if str(c).lower() in cl]
...
def next_renewal_year(m):
    t = date.today()
    return t.year if m >= t.month else t.year + 1
...
async def spark_tufe_guncelle(key:str=Query(...), source:str=Query(...), rate:float=Query(...), year:int=Query(...), month:int=Query(...)):
    ...
    if not expected:
        raise HTTPException(500, "SPARK_TUFE_KEY ayarlanmamış.")
    if key != expected:
    ...
    if not save_cache(data):
        raise HTTPException(500, "Veri Redis'e kaydedilemedi.")
...
async def kira_hesapla(mevcut_kira:float=Form(...), yenileme_ayi:int=Form(...)):
    if mevcut_kira <= 0 or not 1 <= yenileme_ayi <= 12:
        raise HTTPException(400, "Geçerli kira ve yenileme ayı gir.")
    try:
        tufe = get_current_tufe()
    except Exception as e:
        raise HTTPException(503, f"Güncel TÜFE verisi alınamadı. {e}")
    rate=float(tufe["rate"]); ty=int(tufe["year"]); tm=int(tufe["month"]); period=str(tufe["period"]); source=str(tufe["source"])
    artis = mevcut_kira * rate / 100
    yeni = mevcut_kira + artis
    ry = next_renewal_year(yenileme_ayi)
    target = previous_month(ry, yenileme_ayi)
    current = (ty, tm)
    if current == target:
...
@app.get("/api/otv")
async def api_otv():
    return JSONResponse(get_otv_data())


@app.post("/api/otv/yenile")
async def api_otv_yenile():
    return JSONResponse(refresh_otv_data(force=True))
...
async def excel_motor(komut:str=Form(...), file1:UploadFile=File(...), file2:UploadFile=File(...)):
    out = None
    try:
        if not check_extension(file1.filename,(".xlsx",".xls")) or not check_extension(file2.filename,(".xlsx",".xls")):
            raise HTTPException(400,"Geçerli Excel dosyaları seç.")
        a = pd.read_excel(io.BytesIO(await read_upload_limited(file1)))
        b = pd.read_excel(io.BytesIO(await read_upload_limited(file2)))
        if a.empty or b.empty:
            raise HTTPException(400,"Excel dosyalarında veri bulunamadı.")
        a.columns=[normalize_column_name(c) for c in a.columns]
        b.columns=[normalize_column_name(c) for c in b.columns]
        k=komut.lower().strip()
        if any(w in k for w in ["düşeyara","duseyara","vlookup","birleştir","birlestir","merge","eşleştir","eslestir"]):
            c=select_join_column(a,b,komut)
            result=pd.merge(a,b,on=c,how="left",suffixes=("","_referans"))
        elif any(w in k for w in ["pivot","özet","ozet","grupla","toplam"]):
            cc=find_command_columns(a,komut)
            nums=a.select_dtypes(include="number").columns.tolist()
            if not nums:
                raise HTTPException(400,"Pivot/özet için sayısal sütun bulunamadı.")
            val=next((c for c in cc if c in nums),nums[0])
            idx=next((c for c in cc if c!=val),None) or next((c for c in a.columns if c!=val),None)
            if idx is None:
                raise HTTPException(400,"Pivot için grup sütunu bulunamadı.")
            result=pd.pivot_table(a,values=val,index=idx,aggfunc="sum",fill_value=0).reset_index()
        else:
            result=a.copy()
        out=unique_output_path("xlsx")
...
async def resim_pdf_motor(images:List[UploadFile]=File(...)):
    ...
    try:
        for f in images:
            if not f.filename or not check_extension(f.filename,(".jpg",".jpeg",".png")):
                continue
            d=await read_upload_limited(f)
            total += len(d)
            if total > MAX_IMAGES_TOTAL_SIZE:
                raise HTTPException(413,"Seçilen fotoğrafların toplam boyutu çok büyük. Maksimum 100 MB.")
            ...
            im.close()
        if not pil:
            raise HTTPException(400,"Geçerli fotoğraf bulunamadı.")
        out=unique_output_path("pdf")
        pil[0].save(out,"PDF",save_all=True,append_images=pil[1:],resolution=150.0)
...
async def pdf_excel_motor(pdf_file:UploadFile=File(...)):
    out=None
    try:
        if not check_extension(pdf_file.filename,(".pdf",)):
            raise HTTPException(400,"Geçerli bir PDF dosyası seç.")
        rows=[]
        ...
        if not rows:
            raise HTTPException(400,"PDF içinde Excel'e aktarılacak metin veya tablo bulunamadı. Taranmış/resim PDF ise OCR gerekir.")
        mc=max(map(len,rows))
        df=pd.DataFrame([r+[""]*(mc-len(r)) for r in rows])
        out=unique_output_path("xlsx")
...
<script>
let otvData={vehicles:[],unresolved:[],limit:2873900},activeBrand=null;
...
function muaf(v){return v.exempt_price?tl(v.exempt_price):'Hesaplanmıyor'}
function verifiedRow(v){let meta=[v.trim,v.engine,v.transmission,v.fuel].filter(Boolean).join(' · ');return `<div class="row"><div class="rowtop"><div><div class="name">${v.brand} ${v.model}</div><div class="meta">${meta}</div></div><span class="tag">Uygun</span></div><div class="rowprices"><div>Liste Fiyatı<b>${tl(v.price)}</b></div><div class="m">Muaf Fiyatı<b>${muaf(v)}</b></div></div><div class="note">Yerlilik: %${v.locality||'—'} · Kaynak: ${v.source_name||v.brand} · Son kontrol: ${v.checked_at||'—'}</div></div>`}
function parsePending(s){let parts=String(s||'').split(' — '),head=parts[0]||'',detail=parts[1]||'';let brands=['Hyundai','Renault','Fiat','Toyota','Togg'];let brand=brands.find(b=>head.toLowerCase().startsWith(b.toLowerCase()))||head.split(' ')[0]||'';let model=head.slice(brand.length).trim();return {brand,model,detail,raw:s}}
function pendingRows(){return (otvData.unresolved||[]).map(parsePending)}
function pendingRow(p){return `<div class="row pending"><div class="rowtop"><div><div class="name">${p.brand} ${p.model}</div><div class="meta">${p.detail}</div></div><span class="tag">Kontrol</span></div><div class="pendingmsg">Paket fiyatı resmî kaynaktan doğrulanıyor</div><div class="note">Bakanlık yerlilik şartını sağlıyor; fiyat kesinleşmeden “uygun” olarak işaretlenmiyor.</div></div>`}
function allBrands(){let a=[...(otvData.vehicles||[]).map(v=>v.brand),...pendingRows().map(v=>v.brand)];return [...new Set(a.filter(Boolean))]}
...
function renderOTV(){document.getElementById('otvLimit').textContent=tl(otvData.limit);buttons();let verified=(otvData.vehicles||[]).filter(v=>!activeBrand||v.brand===activeBrand);let pending=pendingRows().filter(v=>!activeBrand||v.brand===activeBrand);document.getElementById('otvSummary').textContent=`${verified.length} fiyatı doğrulanan paket · ${pending.length} fiyatı doğrulanan paket bekliyor · Son güncelleme ${otvData.updated_at||'—'}`;let html='';if(verified.length)html+=verified.map(verifiedRow).join('');if(pending.length)html+=pending.map(pendingRow).join('');document.getElementById('otvList').innerHTML=html||'<div class="panel">Bu markada kayıt bulunamadı.</div>'}
