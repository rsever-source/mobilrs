...
function newsCard(item){let u=safeUrl(item.url);let title=esc(item.title),summary=esc(item.summary),source=esc(item.source||'Kaynak belirtilmemiş'),cat=esc(item.category||'Gündem'),dt=esc(newsDate(item.published_at));return '<article class="news-card"><div class="news-meta"><span class="news-tag">'+cat+'</span><span class="news-date">'+dt+'</span></div><h3>'+title+'</h3><p class="news-summary">'+summary+'</p><div class="news-bottom"><span class="news-source">Kaynak: <b>'+source+'</b></span>'+(u?'<a class="news-link" href="'+esc(u)+'" target="_blank" rel="noopener noreferrer">Habere Git ↗</a>':'')+'</div></article>'}
