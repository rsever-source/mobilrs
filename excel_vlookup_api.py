...
<section id="excel" class="page"><div class="pagehead"><button class="back" onclick="openPage('home')">‹</button><h2>Excel</h2></div><div class="panel"><form action="/excel-islem" method="post" enctype="multipart/form-data"><div class="filebox">📄 <span id="excelFileName2">Dosya seçilmedi — Dosya Seç</span><input type="file" name="file2" accept=".xlsx,.xls" required onchange="excelDosya2Secildi(this)"></div><label class="field">İşlem</label><textarea name="komut" placeholder="Örn: Dosyaları Musteri_ID sütunundan düşeyara yap." required></textarea><button class="btn">Excel İşlemini Başlat</button></form></div></section>
...
function renderHome(){document.getElementById('heroCount').textContent=(otvData.vehicles||[]).length+' paket';document.getElementById('heroBrands').textContent=allBrands().length+' marka';document.getElementById('heroTime').textContent=otvData.updated_time||'—'}
function pdfSecildi(input){let el=document.getElementById('pdfFileName');if(input.files&&input.files.length){el.textContent='✓ PDF seçildi: '+input.files[0].name;}else{el.textContent='PDF seçilmedi — PDF Dosyasını Seç';}}function excelDosya1Secildi(input){let el=document.getElementById('excelFileName1');if(input.files&&input.files.length){el.textContent='✓ Dosya 1 seçildi: '+input.files[0].name;}else{el.textContent='Dosya seçilmedi — Dosya Seç';}} function excelDosya2Secildi(input){let el=document.getElementById('excelFileName2');if(input.files&&input.files.length){el.textContent='✓ Dosya 2 seçildi: '+input.files[0].name;}else{el.textContent='Dosya seçilmedi — Dosya Seç';}}function renderOTV(){document.getElementById('otvLimit').textContent=tl(otvData.limit);buttons();let rows=(otvData.vehicles||[]).filter(v=>!activeBrand||v.brand===activeBrand);document.getElementById('otvSummary').textContent=`${rows.length} uygun paket · Son fiyat kontrolü ${otvData.updated_at||'—'}`;document.getElementById('otvList').innerHTML=rows.length?rows.map(verifiedRow).join(''):'<div class="panel">Bu markada uygun ve fiyatı doğrulanmış paket bulunamadı.</div>'}
