...
.vehicle-grid{display:grid;grid-template-columns:repeat(2,minmax(0,390px));justify-content:center;gap:12px}.vehicle-card{border:1px solid var(--line);border-radius:12px;background:#fff;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;gap:10px;min-width:0}.vehicle-card .vehicle-info{min-width:0}.vehicle-card .brandline{font-size:10px;color:var(--muted);font-weight:750;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.vehicle-card h3{margin:3px 0 2px;font-size:15px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.vehicle-card .trim{font-size:11px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.vehicle-card .price-line{flex:0 0 auto;text-align:right}.vehicle-card .price-label{font-size:9px;color:var(--muted);white-space:nowrap}.vehicle-card .price{font-size:15px;font-weight:850;white-space:nowrap}.vehicle-card .arrow{display:none}
...
function openPage(id){document.querySelectorAll('.page').forEach(x=>x.classList.remove('on'));document.getElementById(id).classList.add('on');let bh=document.getElementById('bottomHome');if(bh)bh.style.display=id==='home'?'none':'';scrollTo(0,0);if(id==='otv')renderOTV()}
