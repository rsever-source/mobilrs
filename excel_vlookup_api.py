...
function verifiedRow(v){return `<div class="row"><div class="rowtop"><div><div class="name">${v.brand} ${v.model}</div><div class="meta">${v.trim||''}</div></div><span class="tag">Uygun</span></div><div class="price">Liste Fiyatı<b>${tl(v.price)}</b></div><div class="note">Yerlilik: %${v.locality||'—'} · Kaynak: ${v.source_name||v.brand} · Son kontrol: ${v.checked_at||'—'}</div></div>`}
...
function renderOTV(){document.getElementById('otvLimit').textContent=tl(otvData.limit);buttons();let rows=(otvData.vehicles||[]).filter(v=>!activeBrand||v.brand===activeBrand);document.getElementById('otvSummary').textContent=`${rows.length} uygun paket · Son güncelleme ${otvData.updated_at||'—'}`;document.getElementById('otvList').innerHTML=rows.length?rows.map(verifiedRow).join(''):'<div class="panel">Bu markada uygun ve fiyatı doğrulanmış paket bulunamadı.</div>'}
