...
let newsData={items:[],updated_at:null},pageStack=['home'];
...
function newsCard(item){let u=safeUrl(item.url),title=esc(item.title),summary=esc(shortNewsSummary(item.summary)),source=esc(item.source||'Kaynak belirtilmemiş'),dt=esc(newsDate(item.published_at)),idx=newsData.items.indexOf(item);return '<article class="news-card clickable" data-news-index="'+idx+'"><div class="news-meta"><span class="news-date">'+dt+'</span></div><h3>'+title+'</h3><p class="news-summary">'+summary+'</p><div class="news-bottom"><span class="news-source">Kaynak: <b>'+source+'</b></span>'+(u?'<span class="news-link">Haberi aç ↗</span>':'')+'</div></article>'}
...
function openNewsDetail(index){let item=newsData.items[index];if(!item)return;document.getElementById('newsDetailTitle').textContent=item.title||'';document.getElementById('newsDetailSummary').textContent=item.summary||'';let u=safeUrl(item.url),source=esc(item.source||'Kaynak belirtilmemiş');document.getElementById('newsDetailSource').innerHTML=u?('Kaynak: <a href="'+esc(u)+'" target="_blank" rel="noopener noreferrer">'+source+' ↗</a>'):('Kaynak: '+source);openPage('newsDetail')}
...
document.addEventListener('touchstart',e=>{if(e.touches.length===1){touchStartX=e.touches[0].clientX;touchStartY=e.touches[0].clientY}},{passive:true});
document.addEventListener('touchend',e=>{if(e.changedTouches.length!==1)return;let dx=e.changedTouches[0].clientX-touchStartX,dy=e.changedTouches[0].clientY-touchStartY;if(Math.abs(dx)>70&&Math.abs(dx)>Math.abs(dy)*1.3&&dx<0)goPreviousPage()},{passive:true});
