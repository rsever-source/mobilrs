...
.vehicle-main{display:flex;align-items:center;gap:11px;min-width:0}.vehicle-mini{width:78px;height:54px;flex:0 0 78px;border-radius:12px;overflow:hidden;background:linear-gradient(135deg,#edf3ff,#f7f9fc);display:grid;place-items:center;font-size:25px}.vehicle-mini img{width:100%;height:100%;object-fit:cover;display:block}.homecar .carpic{height:72px;border-radius:14px;background:linear-gradient(135deg,#eaf1ff,#f7f9fc);display:grid;place-items:center;overflow:hidden;margin-bottom:10px;font-size:30px}.homecar .carpic img{width:100%;height:100%;object-fit:cover;display:block}.homecar .carbrand{font-size:11px;color:var(--muted);font-weight:650}.homecar .carmodel{font-size:14px;font-weight:750;line-height:1.3;margin-top:3px;overflow-wrap:anywhere}.homecar .carprice{font-size:12px;color:var(--blue);font-weight:750;margin-top:7px}.row .name{font-size:15px;font-weight:750}.row .meta{font-size:13px}.p b{font-size:16px;font-weight:700}.p small{font-size:11px}.sec h3{font-size:20px;font-weight:700}.hero h2{font-weight:750}.sub{font-size:13px}.herofoot small{font-size:11px}.hero button,.btn{font-weight:700}</style></head><body><main class="app">
...
function otvRateFor(v){let p=Number(v.price||0),b=(v.brand||'').toUpperCase(),m=(v.model||'').toUpperCase();if(!p)return null;let solve=(rules)=>{for(let [r,min,max] of rules){let base=p/1.20/(1+r);if(base>min&&(max==null||base<=max))return r}return null};if(b==='TOGG')return solve([[.25,0,1650000],[.55,1650000,null]]);if(b==='TOYOTA'&&m.includes('C-HR'))return solve([[.70,0,1250000],[.80,1250000,null]]);if(b==='TOYOTA'&&m.includes('COROLLA'))return solve([[.75,0,850000],[.80,850000,1100000],[.90,1100000,1650000],[1.00,1650000,null]]);if(b==='FIAT'&&m.includes('ULYSSE'))return solve([[1.50,0,1650000],[1.70,1650000,null]]);if(b==='FIAT'&&m.includes('EGEA'))return solve([[.75,0,850000],[.80,850000,1100000],[.90,1100000,1650000],[1.00,1650000,null]])||solve([[.70,0,650000],[.75,650000,900000],[.80,900000,1100000],[.90,1100000,null]]);return solve([[.70,0,650000],[.75,650000,900000],[.80,900000,1100000],[.90,1100000,null]])}function exemptPrice(v){let r=otvRateFor(v);return r==null?null:Math.round(Number(v.price)/(1+r))}function vehicleImage(v){let b=(v.brand||'').toUpperCase(),m=(v.model||'').toUpperCase();if(b==='RENAULT'&&m.includes('BOREAL'))return 'https://upload.wikimedia.org/wikipedia/commons/4/42/2026_Renault_Boreal_front_view_01.png';if(b==='TOYOTA'&&m.includes('C-HR'))return 'https://toyota-media.ch/__image/a/2248486/alias/xxl/v/4/c/25/ar/16-9/fn/Toyota%20C-HR%202026_01.jpg';if(b==='TOYOTA'&&m.includes('COROLLA'))return 'https://www.toyotaofclermont.com/blogs/6088/wp-content/uploads/2025/10/Clermont-Toyota-exterior.jpg';if(b==='TOGG')return 'https://www.togg.eu/assets/img/68a4514343d1be59b0dab71b_T10X-Range.webp';if(b==='FIAT'&&m.includes('EGEA'))return 'https://cdn.otopodyum.com.tr/other/2023/12/08/egea-sedan.webp';if(b==='FIAT'&&m.includes('ULYSSE'))return 'https://images.autouncle.com/it/car_images/dcb39872-a246-49e8-bd49-fd1eea924d8f_2026-fiat-ulysse-179.jpg';return ''}function verifiedRow(v){let ep=exemptPrice(v),img=vehicleImage(v);return `<div class="row"><div class="rowtop"><div class="vehicle-main"><div class="vehicle-mini">${img?`<img src="${img}" loading="lazy" alt="${v.brand} ${v.model}" onerror="this.parentElement.textContent='🚗';">`:'🚗'}</div><div><div class="name">${v.brand} ${v.model}</div><div class="meta">${v.trim||''}</div></div></div><span class="tag">Uygun</span></div><div class="price">Liste Fiyatı<b>${tl(v.price)}</b></div>${ep?`<div class="price">Tahmini ÖTV Muaf Fiyat<b>${tl(ep)}</b></div>`:''}<div class="note">Yerlilik: %${v.locality||'—'} · Kaynak: ${v.source_name||v.brand} · Son kontrol: ${v.checked_at||'—'}</div></div>`}
