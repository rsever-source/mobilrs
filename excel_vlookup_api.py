...
.page{display:none}.page.on{display:block}.home{padding:30px}.hero{padding:30px;border:1px solid #dce6f6;border-radius:24px;background:linear-gradient(135deg,#f8fbff,#edf4ff)}.eyebrow{font-size:12px;font-weight:800;color:var(--blue);letter-spacing:.08em;text-transform:uppercase;margin-bottom:9px}.hero h1{margin:0;font-size:36px;line-height:1.08;letter-spacing:-1.2px}.hero p{margin:10px 0 0;max-width:680px;color:var(--muted);font-size:15px;line-height:1.55}.hero-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:24px}.stat{background:#fff;border:1px solid #dfe7f3;border-radius:16px;padding:15px 17px}.stat small{display:block;color:var(--muted);font-size:11px}.stat b{display:block;margin-top:4px;font-size:22px;letter-spacing:-.3px}.section{margin-top:28px}.section-head{display:flex;align-items:end;justify-content:space-between;gap:15px;margin-bottom:12px}.section-head h2{margin:0;font-size:21px;letter-spacing:-.4px}.section-head p{margin:0;color:var(--muted);font-size:12px}
.vehicle-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.vehicle-card{border:1px solid var(--line);border-radius:17px;background:#fff;padding:17px;cursor:pointer;transition:.15s ease}.vehicle-card:hover{transform:translateY(-2px);border-color:#bfd0ec;box-shadow:0 10px 25px rgba(20,32,55,.07)}.vehicle-card .brandline{font-size:11px;color:var(--muted);font-weight:750}.vehicle-card h3{margin:5px 0 3px;font-size:16px;line-height:1.25}.vehicle-card .trim{font-size:12px;color:var(--muted);min-height:18px}.vehicle-card .price-line{display:flex;justify-content:space-between;align-items:end;gap:10px;margin-top:16px;padding-top:13px;border-top:1px solid var(--line)}.vehicle-card .price-label{font-size:10px;color:var(--muted)}.vehicle-card .price{font-size:17px;font-weight:850}.vehicle-card .arrow{color:var(--blue);font-size:20px}
...
.tools{margin-top:30px}.tool-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.tool{display:flex;align-items:center;gap:13px;border:1px solid var(--line);border-radius:16px;padding:17px;background:#fff;cursor:pointer}.tool-icon{width:42px;height:42px;flex:0 0 42px;border-radius:12px;background:var(--soft-blue);display:grid;place-items:center;color:var(--blue);font-weight:900}.tool b{font-size:14px}.tool small{display:block;margin-top:3px;color:var(--muted);font-size:11px}
...
@media(max-width:640px){body{background:#fff}.app{width:100%;margin:0;border:0;border-radius:0;box-shadow:none;min-height:100vh}.header{padding:17px 16px}.brand{font-size:22px}.header-note{font-size:10px}.home,.page-wrap{padding:18px 16px 80px}.hero{padding:20px;border-radius:20px}.hero h1{font-size:28px}.hero p{font-size:13px}.hero-stats{grid-template-columns:1fr 1fr;gap:8px}.stat:last-child{grid-column:1/-1}.stat b{font-size:20px}.section{margin-top:24px}.section-head h2{font-size:19px}.vehicle-grid{grid-template-columns:1fr;gap:9px}.vehicle-card{padding:15px}.vehicle-card .price-line{margin-top:13px;padding-top:11px}.tool-grid{grid-template-columns:1fr}.tool{padding:15px}.page-head h1{font-size:23px}.list{grid-template-columns:1fr}.row{padding:15px}.bottom-home{display:block;position:fixed;left:50%;bottom:12px;transform:translateX(-50%);border:1px solid var(--line);background:#fff;color:var(--blue);border-radius:999px;padding:10px 17px;font-weight:850;box-shadow:0 8px 25px rgba(0,0,0,.12);z-index:20}}
</style></head><body><main class="app">
<header class="header"><div class="brand">Engelli<span>.me</span></div><div class="header-note">Bilgi ve hesaplama araçları</div></header>
<section id="home" class="page on"><div class="home">
<div class="hero"><div class="eyebrow">Güncel fiyat bilgisi</div><h1>Güncel ÖTV muaf araçlar</h1><p>Liste fiyatları ve hesaplanmış fiyatlar tek yerde. Uygun araçları marka ve paket bazında inceleyebilirsiniz.</p><div class="hero-stats"><div class="stat"><small>Uygun paket</small><b id="heroCount">—</b></div><div class="stat"><small>Marka</small><b id="heroBrands">—</b></div><div class="stat"><small>Son fiyat kontrolü</small><b id="heroTime">—</b></div></div><div class="more"><button class="primary" onclick="openPage(\'otv\')">Güncel listeyi gör →</button></div></div>
<div class="section"><div class="section-head"><div><h2>Güncel araçlar</h2><p>Liste fiyatına göre öne çıkan paketler</p></div></div><div id="homeVehicles" class="vehicle-grid"></div></div>
<div class="tools"><div class="section-head"><div><h2>Diğer araçlar</h2><p>Yeni özellikler burada yer alacak</p></div></div><div class="tool-grid"><div class="tool" onclick="openPage(\'kira\')"><div class="tool-icon">₺</div><div><b>Kira Hesaplama</b><small>TÜFE ile kira artışını hesapla</small></div></div><div class="tool" onclick="openPage(\'excel\')"><div class="tool-icon">X</div><div><b>Excel İşlemleri</b><small>Düşeyara, birleştirme ve pivot</small></div></div><div class="tool" onclick="openPage(\'pdfexcel\')"><div class="tool-icon">PDF</div><div><b>PDF → Excel</b><small>PDF içeriğini Excel\'e aktar</small></div></div></div></div>
</div></section>
...
function renderHomeVehicles(){let el=document.getElementById("homeVehicles"),all=(otvData.vehicles||[]).filter(v=>Number(v.price)>0),byModel={};all.forEach(v=>{let key=((v.brand||"")+" "+(v.model||"")).trim().toUpperCase();if(!byModel[key]||Number(v.price)<Number(byModel[key].price))byModel[key]=v});let vs=Object.values(byModel).sort((a,b)=>Number(a.price)-Number(b.price)).slice(0,6);el.innerHTML=vs.length?vs.map(v=>\'<div class="vehicle-card" onclick="openPage(\\\'otv\\\')"><div class="brandline">\'+(v.brand||\'\')+\'</div><h3>\'+(v.model||\'\')+\'</h3><div class="trim">\'+(v.trim||\'\')+\'</div><div class="price-line"><div><div class="price-label">Liste fiyatı</div><div class="price">\'+tl(v.price)+\'</div></div><div class="arrow">→</div></div></div>\').join(\'\'):\'<div class="panel">Araç listesi yükleniyor…</div>\'}
function renderHome(){renderHomeVehicles();let c=document.getElementById(\'heroCount\'),b=document.getElementById(\'heroBrands\'),t=document.getElementById(\'heroTime\');c.textContent=(otvData.vehicles||[]).length+\' paket\';b.textContent=allBrands().length+\' marka\';t.textContent=otvData.updated_time||\'—\'}
